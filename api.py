...
def handle_dialog(req, res):
    ...
    if 'прес' in req['request']['original_utterance'].lower():
        res['response']['text'] = 'Лягте на пол, вытянув ноги. Расположите руки на полу по бокам для поддержки. Сокращая мышцы нижнего пресса, поднимайте ноги под прямым углом к полу. Выполняйте упражнение до легкого жжения в области пресса.'
        return
    ...
    if 'сниж' or 'веса', 'похуд' in req['request']['original_utterance'].lower():
        res['response']['text'] = 'Чтобы избавиться от лишнего веса, отлично подойдет планка. Примите упор лежа, затем согните руки в локтях и обопритесь на предплечья. Увеличивайте время выполнение этого упражнения с каждым днем, и вы сможете добиться результата.'
